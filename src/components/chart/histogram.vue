...
export default {
  ...
  methods: {
    histogramCart1 () {
      let myChart = this.chartMap["histogram"];
      if (typeof myChart === "undefined" || myChart === null) {
        myChart = this.$echarts.init(this.$refs['histogram']);
        this.$set(this.chartMap, "histogram", myChart);
      }
    ...
    historyList () {
      ...
      Api.listPropHistory(param).then(res => {
        ...
        LineChartData.forEach(element => {
          ...
          this.yAxisChartData.push(element.propValue == true ? 1 : 0)
          this.histogramCart1()
        });
      })
    },
